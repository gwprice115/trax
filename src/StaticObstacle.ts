...
export class StaticObstacle extends Phaser.Physics.Arcade.Sprite {
    ...
    constructor(scene: SkiFreeScene, x: number, y: number, setDepthFunction: (y: number) => number, texture: string) {
        ...
        this.displayHeight =
            this.gameScene.getSizeWithPerspective(this.y, this.height) - Math.random() * 0.3 * this.height; // random small variations in size
